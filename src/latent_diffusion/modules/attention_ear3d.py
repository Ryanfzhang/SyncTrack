...
class XFormersCrossInstrAttnProcessor:
    r"""
    Default processor for performing attention-related computations.
    """
    ...
    def __call__(
        ...
        residual = hidden_states #  [B*S=20, F*T=1024, c=256]
        # ori shape [20, 256, 128, 8] T=128 F=8

        batch_instr, c, t, f = shape
        s = 4
        b = batch_instr // s
        ...
        # print('Before: query', query.shape, 'key', key_raw.shape, 'value', value_raw.shape) # [20, 1024, 256]
        key = rearrange(key, "(b s) (t f) c -> (b t) (s f) c", s=s, t=t)  # [5*128, 4*8, 256]
        value = rearrange(value, "(b s) (t f) c -> (b t) (s f) c", s=s, t=t)
        query = rearrange(query, "(b s) (t f) c -> (b t) (s f) c", s=s, t=t) # torch.Size([640, 32, 256])
        # print('After: query', query.shape, 'key', key.shape, 'value', value.shape)

        query = attn.head_to_batch_dim(query).contiguous() # torch.Size([5120, 32, 32])
        key = attn.head_to_batch_dim(key).contiguous()
        ...
        # dropout
        hidden_states = attn.to_out[1](hidden_states) # [b*t=640, s*f=32, c=256]
        hidden_states = rearrange(hidden_states, "(b t) (s f) c -> (b s) (t f) c", t=t, s=s) # [20, 1024, 256]
        attn.residual_connection = False
        if attn.residual_connection:
            hidden_states = hidden_states + residual
...
class CustomAttention(Attention):
    def set_use_memory_efficient_attention_xformers(
        ...
        processor = XFormersCrossInstrAttnProcessor()
        self.set_processor(processor)
        print("using xformers attention processor for multi instr attention")
...
class CustomTransformerBlock(nn.Module):
    def __init__(
        ...
        self.attn_instr = CustomAttention( 
            query_dim=dim, # 320 C
            heads=n_heads, # 5
            dim_head=d_head, # 64
            dropout=dropout, # 0.0
        ...
        self.norm1 = nn.LayerNorm(dim)
        self.norm_instr = nn.LayerNorm(dim)
        self.norm2 = nn.LayerNorm(dim)
        self.norm3 = nn.LayerNorm(dim)
        self.checkpoint = checkpoint

        self.attn_instr.set_use_memory_efficient_attention_xformers(True)
    ...
    def _forward(self, x, context=None):
        if x.ndim == 4 and self.attn_instr is not None:
            x_shape = x.shape
            x = rearrange(x, "b c t f -> b (t f) c") # [4B, H*W, C]
        x = self.attn1(self.norm1(x)) + x
        if self.attn_instr is not None:
            x = self.attn_instr(self.norm_instr(x), shape=x_shape) + x
        x = self.attn2(self.norm2(x), context=context) + x
        x = self.ff(self.norm3(x)) + x
        return x
