...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        legacy=True,
        no_condition=False,
        num_sync_blocks=1  # Number of output blocks to use SyncSpatialTransformer wok
    ):
        super().__init__()
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                if ds in attention_resolutions:
                    ...
                    # elif 8-count>num_sync_blocks: # wok
                    elif count >= total_attention_blocks - num_sync_blocks:
                        # 最后num_sync_blocks个使用SyncSpatialTransformer
