...
class CustomTransformerBlock(nn.Module):
    def __init__(
        ...
        t, f = image_size
        d_hid = 4 * f
        self.attn_inst = torchTransformerEncoderLayer(d_model=f, nhead=1, dim_feedforward=d_hid, 
    ...
    def _forward(self, x, context=None):
        ...
        if self.image_size is not None:
            ...
            batch = B_inst // 4
            instr = 4
            # # print("image_size", self.image_size, "shape", shape)
            # B_inst, C, T, F = shape # [B*4, C, T, F] [64, 256, 128, 8]
            # batch = B_inst // 4 # B=16
            # instr = 4
            x = rearrange(x, '(b s) (t f) c -> (b t c) s f', s=instr, c=C, b=batch, f=F, t=T)  # [B*4*C, S, F] [204800, 4, 64]
            # print(x.shape, batch*T*C)
            n = x.size(0)
            chunk_size = 2048
            ...
            for i in range(n_chunks):
                ...
                outputs.append(self.attn_inst(chunk))
            outputs = torch.cat(outputs, dim=0)
            x = outputs + x
            del outputs
            x = rearrange(x, '(b t c) s f -> (b s) (t f) c', s=instr, c=C, b=batch, f=F, t=T)
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        for block in self.transformer_blocks:
            x = block(x, context=context) #, shape=x_shape)   
        # 恢复原始形状 [4B, C, H, W]
...
if __name__ == "__main__":
    x = torch.randn((5, 4, 32, 4, 4)).to("cuda")
    b, s, c, h, w = x.size()
    ...
    transformer = SpatialTransformer(
                    ...
                    d_head=8,
                    depth=2,
                    instr_attn_dim=4,  # 等于输入数据的 freq
                    instr_n_heads=2
                ).cuda()
    x_out = transformer(x)
    print("after", x_out.shape)
